...
import pandas as pd
import numpy as np
import pingparser.core.general as genparse
import pyfun.bamboo as boo
import pyfun.timestrings as timestr
import pingparser.core.runningval as runningval
import warnings

from math import isfinite #test whether nan


#variant cases:
#10.16.24 -- is_warmup variable introduced (but unused). Can ignore
#10.22.24 -- warmup_state: integer which counts number of warmup processes occurring for a given trial.
#         0 = not warm up trial.
...
# TODO: for 11/16/24 and earlier, need to extract CueAlpha_WMOn, and CueAlpha_WMOff manually
...
class Extractor:
    ...
    COLUMN_DTYPES = {
        ...
        'RespPause': 'float64',
        'Resp_time': 'object', #new in v10
        'reward_max_ms': 'Int64',
        'sleap_body_score':  'float64',
        'sleap_snout_score': 'float64',
        'TrialDur': 'float64',
    ...
    def extract(self, fn, sess_name):
        ...
        for TrialNum, df_trial in df_sess_raw.groupby('TrialNum'):
            #grab fixation time, number of fixation interrupted etc.
            if bugged_fixation:
            ...
            #grab response time
            resp_events = self._trial_response_events(df_trial)

            one_trial = self._trial_summarizer(df_trial, fix_events, resp_events)
    ...
    def _post_processor(self, df):
        ...
        # TODO: converstion to more human readable time string (mmyydd) not quite working, so time stamps
        # are left in pd.datetime format (e.g. 1900-01-01 12:18:33.857689600). Upside: allows diff comparisons directly
        """
        try:
            for t_event in ['FixationStarted_time', 'FixationCompleted_time', 'Response_time']:
    ...
    def _trial_response_events(self, df_trial):

        resp_events = {'Resp_time'  : None}

        resp_made = boo.slice(df_trial, {'Subject': ['RespError_cuefrac']}, '+')

        if not resp_made.empty:
            v = resp_made['Value'].iloc[0]
            try:
                if isfinite(float(v)):
                    resp_events['Resp_time'] = resp_made['Timestamp'].iloc[0]
    ...
    def _trial_summarizer(self, df_trial, fix_events, resp_events):
        """
        ...
        for event_name, val in fix_events.items():
            row_holder.loc[event_name, 'val'] = val

        for event_name, val in resp_events.items():
            row_holder.loc[event_name, 'val'] = val
